...
const EclipseWorldMap2 = ({ onNavigate }) => {
  ...
  return (
    <div className={`world-container-history history-theme ${isLightning ? 'lightning-flash' : ''}`}>
            ...
            <div key={level} className={`node-wrapper ${posClass}`}>
              {isCurrent && (
                <div className="void-pedestal">
                  <div className="void-aura-history"></div>
                  <div className="void-tag-history">VOID</div>
              ...
              <div className={`orb-node ${isCurrent ? 'active' : ''} ${isCompleted ? 'done' : 'locked'}`}>
                <div className="orb-inner-history">
                  <span className="orb-num">{level}</span>
                </div>
                {(isCurrent || isCompleted) && <div className="energy-rings-history"></div>}
              </div>
              {level < 50 && <div className="tendril-up-history"></div>}
            </div>
          );
        })}
      </div>
...
export default EclipseWorldMap2;